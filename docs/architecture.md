...
- **Policy API:** Azure Functions-compatible Python service. Locally, it uses seeded intel, optional normalized feed state, and file-backed audit/feed-state logs. In Azure, it can be connected to Cosmos DB, Key Vault, and App Insights.
- **Feed sync:** GitHub malware advisory and OSV malware ingestion writes a normalized feed cache. Hosted feeds are HTTPS-only, reject redirects, and are capped at 5 MiB.
...
flowchart LR
  ...
  Policy --> Intel["Seeded, hosted, or synced intel"]
  Policy --> Registry["npm registry provenance"]
  Sync["Timer/manual feed sync"] --> Intel
  API --> Audit["Audit store"]
  Intel --> FeedState["Feed state"]
  FeedState --> API
  Audit --> Dashboard
```
...
- Feed freshness and sync state are written to `.pounce-sentinel/feed-state.json` by default.
- `services/policy-api/run_local.py sync-feeds` can refresh public feed state when network access is available.
...
- Cosmos DB stores verdicts, exceptions, and feed state in `verdicts`, `exceptions`, and `feed_state`.
